    ...
    T: MatElem,
{
    ...
    pub fn backprop(&mut self, prediction: Mat<T>, truth: Mat<T>) {
        ...
        for i in (0..self.architecture.len() - 1).rev() {
            ...
            self.weights[i] = self.weights[i].sub(&gradients.dot(&self.data[i].transpose()));
            self.biases[i] = self.biases[i].sub(&gradients);
        ...
        T: std::fmt::Display,
    {
        ...
        for i in 1..=epochs {
            ...
            if epochs < 20 || i % (epochs / 20) == 0 {
                ...
                println!("epoch: {i:0>width$} / {epochs:0>width$} ;\tloss: {}", loss);
            }
        }
    }
}
